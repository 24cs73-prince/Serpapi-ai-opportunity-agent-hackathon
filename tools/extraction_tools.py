...
from tools.search_tools import SearchResponse, JobResult, WebResult
...
def extract_opportunities_from_search(items: list | SearchResponse) -> list[Opportunity]:
    ...
    Accepts SearchResponse, list of JobResult, list of WebResult, or list of dicts/Opportunity objects.
    """
    if isinstance(items, SearchResponse):
        if items.jobs:
            return extract_opportunities_with_llm(items)
        elif items.web_results:
            return extract_opportunities_from_search(items.web_results)
        return []
    ...
    if isinstance(items, list):
        ...
        for item in items:
            if isinstance(item, JobResult):
                ...
                web_opps.append(Opportunity(
                    title=item.title,
                    company=item.display_source,
                    description=item.snippet,
                    source="Google Search",
                    application_url=item.link or None,
                    required_skills=_extract_skills_from_description(item.snippet),
                ...
                jobs.append(JobResult(
                    ...
                    apply_link=item.get("apply_link") or item.get("link") or item.get("url"),
